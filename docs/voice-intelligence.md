...
SiaoCut 将语音节奏、音频质量和说话人信息整理为可定位证据。本文说明当前源码中的功能；可下载版本及验收边界以[发布状态](release-updates.md)为准。分析在本机运行，不向远程服务上传媒体，也不会自动删除内容、改写字幕或应用剪辑。
...
- 快速转写只对通过原始媒体时间轴验收的 whisper.cpp 运行时启用 VAD；其他运行时自动使用无 VAD 安全路径。两条路径都会在写入前校验字幕段和词级时间，没有可信词级时间的非空结果会被拒绝，不会按字符平均分配时间。
