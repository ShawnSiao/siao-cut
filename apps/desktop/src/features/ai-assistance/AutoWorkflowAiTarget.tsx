...
export default function AutoWorkflowAiTarget({ codexReady, onChange }: Props) {
  ...
  return <section className="auto-ai-target" aria-label="字幕翻译执行方式">
    ...
    {mode === "api" && service && <div className="auto-ai-fields"><label><span>服务</span><select value={service.id} onChange={(event) => { const next = services.find((item) => item.id === event.target.value); setServiceId(event.target.value); setModelId(next?.modelId ?? ""); setConfirmed(false); }}>{services.map((item) => <option key={item.id} value={item.id}>{item.displayName}</option>)}</select></label><label><span>本次模型</span><input value={modelId} onChange={(event) => { setModelId(event.target.value); setConfirmed(false); }}/></label></div>}
