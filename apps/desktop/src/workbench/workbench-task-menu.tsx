...
export default function WorkbenchTaskMenu({ inputs, actionsFor, error, onRefresh }: { error?: string | null; onRefresh?: () => void; inputs: WorkbenchActivityInputs; actionsFor: (activity: WorkbenchActivity) => WorkbenchActivityAction[] }) {
  ...
  return <details ref={ref} className={`workspace-tasks${attention ? " needs-attention" : ""}`} onKeyDown={(event) => {
    ...
    <div className="workspace-tasks-popover">
      ...
      {error && <p role="alert">{error}<button onClick={onRefresh}>{zh ? "重新查询" : "Retry query"}</button></p>}
