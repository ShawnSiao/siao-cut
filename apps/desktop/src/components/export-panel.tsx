...
const ExportPanel = forwardRef<HTMLElement, Props>(function ExportPanel(props, ref) {
  ...
  return <aside ref={ref} className={`export-panel ${embedded ? "embedded" : ""}`} aria-label={tr("app.s0392")}>
    ...
    <footer className="export-panel-actions">
      <Button disabled={busy || selectedTranslationPending || (selectedTranslationStale && !confirmStaleTranslation) || transcriptionExportBlocked} onClick={onExportTranscript}><Download size={15}/>{tr("app.s0432")}</Button>
      <Button variant="primary" disabled={!canExportVideo || busy || selectedTranslationPending || (selectedTranslationStale && !confirmStaleTranslation) || activeExportRunning} title={mediaCapabilityTitle} onClick={onExportVideo}><Film size={15}/>{tr("app.s0259")}</Button>
    </footer>
  </aside>;
});
