...
export function RuntimeChecklist({ runtime, modelPath, modelAvailable = Boolean(modelPath), onChooseModel, compact = false }: {
    ...
    const items = [
        ...
        { icon: Download, label: tr("app.s0513"), ok: runtime?.ytDlpConfigured ?? false, detail: runtime?.ytDlpConfigured ? "yt-dlp 2026.08.19" : tr("app.s0587") },
    ];
...
export function SegmentRow({ editingSession, projectId, segment, speaker, speakerManual, selected, active, translation, translationLanguage, onSelect, onSave, onSaveTranslation, onSplitAt, onMergePrevious }: {
    ...
    <div><textarea rows={1} value={draft} data-dirty={draft.trim() !== segment.text} onChange={(event) => setDraft(event.target.value)} onCompositionStart={() => { composing.current = true; editingSession?.composition(sourceKey, true); }} onCompositionEnd={() => { composing.current = false; editingSession?.composition(sourceKey, false); }} onFocus={() => { if (!active)
        ...
        if (!event.ctrlKey && !event.metaKey && !event.altKey && !event.shiftKey && event.key === "Enter" && textarea.selectionStart === textarea.selectionEnd) {
            const offset = Array.from(draft.slice(0, textarea.selectionStart)).length;
            if (offset > 0 && offset < Array.from(draft).length) {
                event.preventDefault();
                suppressBlurSaveRef.current = true;
                onSplitAt(draft, offset);
            }
            return;
        }
        if (!event.ctrlKey && !event.metaKey && !event.altKey && event.key === "Backspace" && textarea.selectionStart === 0 && textarea.selectionEnd === 0) {
            event.preventDefault();
            suppressBlurSaveRef.current = true;
            onMergePrevious(draft);
        }
    }} onClick={(event) => event.stopPropagation()} aria-label={tr("app.s0638", { "0": formatTime(segment.start) })} title={tr("app.s0639")}/>{sourceState && <FieldSaveStatus status={sourceState.status} journaled={sourceState.journaled}/>}{translatedSegment && <div className={`translation-editor ${translatedSegment.status}`}><textarea rows={1} value={translationDraft} data-dirty={translationDraft.trim() !== translated} onChange={(event) => setTranslationDraft(event.target.value)} onCompositionStart={() => editingSession?.composition(translationKey, true)} onCompositionEnd={() => editingSession?.composition(translationKey, false)} onBlur={saveTranslation} onKeyDown={(event) => { if (event.nativeEvent.isComposing || event.keyCode === 229) return; if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "s") { event.preventDefault(); saveTranslation(); } }} onClick={(event) => event.stopPropagation()} aria-label={tr("app.creator.translation.edit", { time: formatTime(segment.start), language: translationLanguage?.toUpperCase() ?? "" })}/>{translationState && <FieldSaveStatus status={translationState.status} journaled={translationState.journaled}/>} {translatedSegment.status === "stale" && <small>{tr("app.s0373")}</small>}{translatedSegment.status === "quality_failed" && <small>{tr("app.creator.translation.qualityFailed")}</small>}</div>}</div>
