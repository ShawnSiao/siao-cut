...
let mockSpeakerPackage: SpeakerPackageStatus = {
  ...
  installed: false,
  verified: null,
  assets: speakerAssets.map((asset) => ({ ...asset, installed: false, verified: null })),
};
...
async function mockRun(args: string[]): Promise<CoreEnvelope> {
  ...
  if (command === "project" && subcommand === "list") {
    ...
    mockSpeakerPackage = { ...mockSpeakerPackage, installed: false, verified: null, assets: speakerAssets.map((asset) => ({ ...asset, installed: false, verified: null })) };
    return { apiVersion: "0.1", status: "ok", projects: mockProjects };
  }
  if (command === "project" && subcommand === "show") {
    mockProject = mockProjects.find((project) => project.id === args[2]) ?? mockProject;
  ...
  if (command === "project" && ["undo", "redo"].includes(subcommand)) {
    mockProject.history.canUndo = subcommand === "redo";
    mockProject.history.canRedo = subcommand === "undo";
    return { apiVersion: "0.1", status: "ok", project: mockProject };
  }
  if (command === "project" && subcommand === "relink") return { apiVersion: "0.1", status: "ok", project: mockProject, message: "已重新定位原片，并校验内容与项目记录一致。" };
  ...
  if (command === "transcript" && subcommand === "edit") {
    ...
    mockProject.transcript.segments = mockProject.transcript.segments.map((segment) => segment.id === segmentId ? { ...segment, text } : segment);
    Object.values(mockProject.translations).forEach((translation) => { translation.status = "stale"; });
    return { apiVersion: "0.1", status: "ok", project: mockProject, message: "原文已更新；已有译文已标记为待更新。" };
  }
  if (command === "transcript" && subcommand === "replace") {
    ...
    mockProject.transcript.segments = mockProject.transcript.segments.map((segment) => {
      if (!segment.text.includes(find)) return segment;
      changedSegments += 1;
      return { ...segment, text: segment.text.replaceAll(find, replacement) };
    });
    if (changedSegments) Object.values(mockProject.translations).forEach((translation) => { translation.status = "stale"; });
    return { apiVersion: "0.1", status: "ok", project: mockProject, changedSegments };
  }
  if (command === "transcript" && subcommand === "split") {
    ...
    const left = { ...current, end: at, text: characters.slice(0, textOffset).join("").trim() };
    const right = { ...current, id: createdId, start: at, text: characters.slice(textOffset).join("").trim() };
  ...
  if (command === "transcript" && subcommand === "offset") {
    ...
    for (const segment of mockProject.transcript.segments) {
      ...
      if (segment.start + delta < 0) throw new Error("偏移后字幕时间无效");
      segment.start += delta;
      segment.end += delta;
    }
    let shifted = 0;
  ...
  if (command === "model" && subcommand === "remove") {
    const model = mockModels.find((item) => item.id === args[2]);
    if (model) { model.installed = false; model.bytesOnDisk = 0; model.verified = null; }
  ...
  if (command === "speaker" && subcommand === "install") {
    mockSpeakerPackage = { ...mockSpeakerPackage, installed: true, verified: true, assets: mockSpeakerPackage.assets.map((asset) => ({ ...asset, installed: true, verified: true })) };
  ...
  if (command === "task" && ["retry", "cancel"].includes(subcommand)) {
    ...
    if (task) task.status = subcommand === "retry" ? "queued" : "canceled";
    return { apiVersion: "0.1", status: "ok", project: mockProject };
  }
  return { apiVersion: "0.1", status: "ok", project: mockProject };
}
