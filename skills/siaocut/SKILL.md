...
siaocut --json task claim --worker codex-1
```

When a task is claimed, produce a response JSON file outside the repository and submit it:

```powershell
siaocut --json task submit <taskId> --worker codex-1 --response "C:\Temp\siaocut-response.json"
```
...
```powershell
siaocut --json task heartbeat <taskId> --worker codex-1 --progress 0.5 --message "正在校对译文"
```
