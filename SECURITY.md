...
Private vulnerability reporting is enabled. Report sensitive findings through [GitHub private vulnerability reporting](https://github.com/ShawnSiao/siao-cut/security/advisories/new). Do not publish credentials, private keys, media, transcripts, local paths, personal data, or working exploit details in a public issue.

当前仓库已启用[GitHub 私密漏洞报告](https://github.com/ShawnSiao/siao-cut/security/advisories/new)。公开 Issue 中不得提交凭据、私钥、媒体、文稿、本机路径、个人信息或可直接利用的攻击细节。

For a potentially sensitive issue:

1. Open a private vulnerability report.
2. Include the affected version or commit, impact, and the smallest safe reproduction.
3. Keep credentials, private keys, user media, transcripts, and personal data out of the report; use synthetic evidence where possible.

对于可能涉及敏感信息的问题：

1. 创建私密漏洞报告。
2. 说明受影响的版本或提交、影响范围和最小安全复现方法。
3. 报告中不得包含凭据、私钥、真实用户媒体、文稿或个人信息；应尽量使用合成证据。
...
The current dependency assessment is documented in [Dependency security](docs/dependency-security.en.md).

当前依赖评估见[依赖安全](docs/dependency-security.md)。
